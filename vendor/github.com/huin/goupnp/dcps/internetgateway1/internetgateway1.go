...
const (
	...
	URN_WANOccernetLinkConfig_1    = "urn:schemas-upnp-org:service:WANOccernetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Occernet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Occernet
//
...
// WANOccernetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANOccernetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANOccernetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANOccernetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANOccernetLinkConfig1Clients() (clients []*WANOccernetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANOccernetLinkConfig_1); err != nil {
		return
	}
	clients = newWANOccernetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANOccernetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANOccernetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANOccernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANOccernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANOccernetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANOccernetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANOccernetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANOccernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANOccernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANOccernetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANOccernetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANOccernetLinkConfig1 {
	clients := make([]*WANOccernetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANOccernetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewOccernetLinkStatus: allowed values: Up, Down
func (client *WANOccernetLinkConfig1) GetOccernetLinkStatus() (NewOccernetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewOccernetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANOccernetLinkConfig_1, "GetOccernetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewOccernetLinkStatus, err = soap.UnmarshalString(response.NewOccernetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
