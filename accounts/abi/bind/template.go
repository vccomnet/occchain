...
import "github.com/blockchain/go-blockchain/accounts/abi"
...
import (
	...
	blockchain "github.com/blockchain/go-blockchain"
	"github.com/blockchain/go-blockchain/accounts/abi"
	"github.com/blockchain/go-blockchain/accounts/abi/bind"
	"github.com/blockchain/go-blockchain/common"
	"github.com/blockchain/go-blockchain/core/types"
	"github.com/blockchain/go-blockchain/event"
)
...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		// Deploy{{.Type}} deploys a new Blockchain contract, binding an instance of {{.Type}} to it.
	...
	// {{.Type}} is an auto generated Go binding around an Blockchain contract.
	type {{.Type}} struct {
	...
	// {{.Type}}Caller is an auto generated read-only Go binding around an Blockchain contract.
	type {{.Type}}Caller struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Transactor is an auto generated write-only Go binding around an Blockchain contract.
	type {{.Type}}Transactor struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Filterer is an auto generated log filtering Go binding around an Blockchain contract events.
	type {{.Type}}Filterer struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Session is an auto generated Go binding around an Blockchain contract,
	...
	// {{.Type}}CallerSession is an auto generated read-only Go binding around an Blockchain contract,
	...
	// {{.Type}}TransactorSession is an auto generated write-only Go binding around an Blockchain contract,
	...
	// {{.Type}}Raw is an auto generated low-level Go binding around an Blockchain contract.
	type {{.Type}}Raw struct {
	  Contract *{{.Type}} // Generic contract binding to access the raw methods on
	}

	// {{.Type}}CallerRaw is an auto generated low-level read-only Go binding around an Blockchain contract.
	type {{.Type}}CallerRaw struct {
		Contract *{{.Type}}Caller // Generic read-only contract binding to access the raw methods on
	}

	// {{.Type}}TransactorRaw is an auto generated low-level write-only Go binding around an Blockchain contract.
	...
	{{range .Events}}
		...
		type {{$contract.Type}}{{.Normalized.Name}}Iterator struct {
			...
			sub  blockchain.Subscription // Subscription for errors, completion and termination
...
import org.blockchain.occ.*;
import org.blockchain.occ.internal.*;
...
{{range $contract := .Contracts}}
	public class {{.Type}} {
		...
		{{if .InputBin}}
			...
			// deploy deploys a new Blockchain contract, binding an instance of {{.Type}} to it.
			public static {{.Type}} deploy(TransactOpts auth, BlockchainClient client{{range .Constructor.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
		...
		// Blockchain address where this contract is located at.
		public final Address Address;

		// Blockchain transaction in which this contract was deployed (if known!).
		...
		public {{.Type}}(Address address, BlockchainClient client) throws Exception {
