...
import (
	...
	"github.com/blockchain/go-blockchain/accounts/usbwallet/internal/trezor"
...
// or whether the Blockchain app was not started on it.
...
// Blockchain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.BlockchainAddress)
	if _, err := w.trezorExchange(&trezor.BlockchainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.BlockchainSignTx{
	...
	response := new(trezor.BlockchainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.BlockchainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
