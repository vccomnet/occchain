...
import (
	...
	"github.com/blockchain/go-blockchain/common/mclock"
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Blockchain protocol")
	...
	log.Info("Light Blockchain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Blockchain peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Blockchain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Blockchain peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Blockchain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Blockchain message arrived", "code", msg.Code, "bytes", msg.Size)
