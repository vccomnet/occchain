...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of occ is "occ". If no
...
// These resources are resolved differently for "occ" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by occ 1.4 are used if they exist.
	if c.name() == "occ" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "occ" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-blockchain-keystore")
		ephemeral = keydir
	}
