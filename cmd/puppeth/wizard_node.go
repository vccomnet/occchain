...
func (w *wizard) deployNode(boot bool) {
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 40404, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 40404, peersTotal: 50, peersLight: 0, gasTarget: 7.5, gasLimit: 10, gasPrice: 1}
		}
	}
	existed := err == nil
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an occerbase to mine against
			fmt.Println()
			if infos.occerbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.occerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.occerbase)
				infos.occerbase = w.readDefaultAddress(common.HexToAddress(infos.occerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Blockchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
