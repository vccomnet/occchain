...
RUN \
  echo '(cd ../eth-net-intelligence-api && pm2 start /ethstats.json)' >  explorer.sh && \
	echo '(cd ../occerchain-light && npm start &)'                      >> explorer.sh && \
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.blockchain
    environment:
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.blockchain"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
