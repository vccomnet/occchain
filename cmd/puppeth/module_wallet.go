...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'occ --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec occ --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppethNetworkID/{{.NetworkID}}/g' dist/js/occerwallet-master.js && \
	sed -i 's/PuppethNetwork/{{.Network}}/g'     dist/js/occerwallet-master.js && \
	sed -i 's/PuppethDenom/{{.Denom}}/g'         dist/js/occerwallet-master.js && \
	sed -i 's/PuppethHost/{{.Host}}/g'           dist/js/occerwallet-master.js && \
	sed -i 's/PuppethRPCPort/{{.RPCPort}}/g'     dist/js/occerwallet-master.js
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.RPCPort}}:50505"{{if not .VHost}}
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.blockchain
    environment:
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["50505/tcp"]
