...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 occer` in value to maximum `1 occer` per 24h period
...
implemented for `occ`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
