...
func TestAccountListEmpty(t *testing.T) {
	occ := runGeth(t, "account", "list")
	occ.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	occ := runGeth(t, "account", "list", "--datadir", datadir)
	defer occ.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	occ := runGeth(t, "account", "new", "--lightkdf")
	defer occ.ExpectExit()
	occ.Expect(`
...
`)
	occ.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	occ := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer occ.ExpectExit()
	occ.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(occ.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(occ.StderrText(), m) {
...
// https://github.com/blockchain/go-blockchain/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	occ.SetTemplateFunc("keypath", func(file string) string {
