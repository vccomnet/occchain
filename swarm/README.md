...
[https://swarm.blockchain.org](https://swarm.blockchain.org)

Swarm is a distributed storage platform and content distribution service, a native base layer service of the blockchain web3 stack. The primary objective of Swarm is to provide a decentralized and redundant store for dapp code and data as well as block chain and state data. Swarm is also set out to provide various base layer services for web3, including node-to-node messaging, media streaming, decentralised database services and scalable state-channel infrastructure for decentralised service economies.

[![Travis](https://travis-ci.org/blockchain/go-blockchain.svg?branch=master)](https://travis-ci.org/blockchain/go-blockchain)
[![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/occersphere/orange-lounge?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)
...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/blockchain/go-blockchain

    go install github.com/blockchain/go-blockchain/cmd/swarm
...
To run Swarm you need an Blockchain account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the Blockchain address representing that account. For example:
...
### Blockchain Name Service resolution

The Blockchain Name Service is the Blockchain equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.eth`), `swarm` has to connect to a `occ` instance, which is synced with the Blockchain mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.blockchain/occ.ipc'
...
You must have your working copy under `$GOPATH/src/github.com/blockchain/go-blockchain`.

Most likely you will be working from your fork of `go-blockchain`, let's say from `github.com/nirname/go-blockchain`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-blockchain.git $GOPATH/src/github.com/blockchain/go-blockchain
```
...
```
stateth --rm --grafana-dashboards-folder $GOPATH/src/github.com/blockchain/go-blockchain/swarm/grafana_dashboards --influxdb-database metrics
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The Blockchain Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
...
Their source code can be found at: https://github.com/occersphere/swarm-dapps
...
complex changes though, please check up with the core devs first on [our Swarm gitter channel](https://gitter.im/occersphere/orange-lounge)
 ...
 * [Code review guidelines](https://github.com/blockchain/go-blockchain/wiki/Code-Review-Guidelines).
