...
`pss` is under active development, and the first implementation is yet to be merged to the Blockchain main branch. Expect things to change.
...
The code tutorial [p2p programming in go-blockchain](https://github.com/nolash/go-blockchain-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `go-blockchain` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `go-blockchain` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
