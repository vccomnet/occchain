...
import (
	...
	occmetrics "github.com/blockchain/go-blockchain/metrics"
...
func Setup(ctx *cli.Context) {
	if occmetrics.Enabled {
		...
		go occmetrics.CollectProcessMetrics(2 * time.Second)

		if enableExport {
			log.Info("Enabling swarm metrics export to InfluxDB")
			go influxdb.InfluxDBWithTags(occmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
